...
def oval(x_center, y_center, x_width, y_height, tool_diameter, feed_rate, z_feed_rate, from_z, to_z, step_z,  safety_z,
    ...
    for h in generate_heights(from_z, to_z, step_z):
        for d in generate_excentric(0.00, y_height/2.0, tool_diameter):
            ...
            gcode.append(g2_gcode(x_end_point=point_se.x, y_end_point=point_se.y, spiral_end_altitude=h,
            ...
            gcode.append(g2_gcode(x_end_point=point_nw.x, y_end_point=point_nw.y, spiral_end_altitude=h,
...
def path(path_to_follow, feed_rate, z_feed_rate, from_z, to_z, step_z, safety_z):
    ...
    gcode.append(g0_gcode(x=path_to_follow[0][0], y=path_to_follow[0][1]))
    for h in generate_heights(from_z, to_z, step_z):
        gcode.append(g1_gcode(z=h, feedrate=z_feed_rate))
...
def rectangle(x_dimension, y_dimension, x_mini, y_mini, tool_diameter, feed_rate, z_feed_rate, from_z, to_z, step_z,
    ...
    gcode = list()
    gcode.append(g0_gcode(x=0, y=0, z=safety_z))
    gcode.append(g0_gcode(x=x_mini - tool_diameter / 2, y=y_mini - tool_diameter / 2))
...
def thin(x_dimension, y_dimension, x_mini, y_mini, tool_diameter, feed_rate, z_feed_rate, from_z, to_z, step_z,
    ...
    gcode = list()
    gcode.append(g0_gcode(x=0, y=0, z=safety_z))
    gcode.append(g0_gcode(x=x_center, y=y_center))
    ...
    for h in generate_heights(from_z, to_z, step_z):
        ...
        while nb_turns/2*tool_diameter < x_dimension/2 or nb_turns/2*tool_diameter < y_dimension/2:
            gcode.append(g1_gcode(y=min(y_center + (nb_turns / 2 + 0.5) * tool_diameter,
                                        y_mini + y_dimension - tool_diameter / 2), feedrate=feed_rate))
            gcode.append(g1_gcode(x=min(x_center + (nb_turns / 2 + 0.5) * tool_diameter,
                                        x_mini + x_dimension - tool_diameter / 2), feedrate=feed_rate))
            gcode.append(g1_gcode(y=max(y_center - (nb_turns / 2 + 0.5) * tool_diameter,
                                        y_mini + tool_diameter / 2), feedrate=feed_rate))
            gcode.append(g1_gcode(x=max(x_center - (nb_turns / 2 + 0.5) * tool_diameter,
...
def hole(x_center, y_center, hole_diameter, tool_diameter, feed_rate, from_z, to_z, step_z, safety_z):
    ...
    for h in generate_heights(from_z, to_z, step_z):
        gcode.append(g2_gcode(x_center_offset=hole_diameter / 2 - tool_diameter / 2, y_center_offset=0,
                              spiral_end_altitude=h, feedrate=feed_rate))
...
def cylinder(x_center, y_center, cylinder_diameter, tool_diameter, feed_rate, from_z, to_z, step_z, safety_z):
    ...
    for h in generate_heights(from_z, to_z, step_z):
        gcode.append(g3_gcode(x_center_offset=(cylinder_diameter + tool_diameter) / 2 - tool_diameter / 2,
...
def g2_gcode(x_end_point=None, y_end_point=None, spiral_end_altitude=None, x_center_offset=None, y_center_offset=None,
    ...
    return gcode_format(prefix='G2', x=x_end_point, y=y_end_point, z=spiral_end_altitude, i=x_center_offset,
...
def g3_gcode(x_end_point=None, y_end_point=None, x_center_offset=None, y_center_offset=None, spiral_end_altitude=None,
    ...
    return gcode_format(prefix='G3', x=x_end_point, y=y_end_point, i=x_center_offset, j=y_center_offset,
